...
output "cluster_security_group_arn" {
  ...
  value = try(
    "arn:${data.aws_partition.current.partition}:ec2:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:security-group/${aws_eks_cluster.main[0].vpc_config[0].cluster_security_group_id}",
    null
  )
}
...
output "kubeconfig_command" {
  ...
  value       = var.create_cluster ? "aws eks update-kubeconfig --region ${data.aws_region.current.region} --name ${aws_eks_cluster.main[0].name}" : null
}
